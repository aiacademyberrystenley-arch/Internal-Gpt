...
import { Award, Bot, BookOpen, Building2, CalendarDays, FileText, GraduationCap, LayoutDashboard, LifeBuoy, Lock, LogOut, Menu, MessageSquare, MoreVertical, Paperclip, Plus, Search, Send, Settings, Sparkles, Wallet, X } from 'lucide-react';
...
export default function Chat({ profile, onLogout }) {
  ...
  return (
    <div className="grid h-screen min-h-[640px] lg:grid-cols-[280px_1fr]">
      ...
      {/* Unified sidebar — static column on desktop, slide-over drawer on mobile */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex w-[280px] flex-col border-r border-slate-800 bg-slate-900 transition-transform duration-200 lg:static lg:z-auto lg:w-auto lg:translate-x-0 ${
      ...
      >
        ...
        <div className="flex items-center gap-3 border-b border-slate-800/70 px-4 py-4">
          ...
          <button onClick={() => setSidebarOpen(false)} className="focus-ring rounded-lg p-1.5 text-slate-400 hover:bg-slate-800 lg:hidden" aria-label="Close menu">
            <X size={18} />
          </button>
        </div>
      ...
      <section className="relative flex min-w-0 flex-col bg-slate-950">
        {/* Mobile top bar — opens the drawer; the sidebar is hidden on phones */}
        <div className="flex items-center gap-2 border-b border-slate-800 bg-slate-900 px-3 py-2.5 lg:hidden">
          <button onClick={() => setSidebarOpen(true)} className="focus-ring rounded-lg p-2 text-slate-300 hover:bg-slate-800" aria-label="Open menu">
            <Menu size={20} />
          </button>
          <div className="flex min-w-0 flex-1 items-center gap-2">
            <img src="/logo.png" alt="SRM IST" className="h-7 w-7 shrink-0 rounded-full bg-white object-contain" />
            <h1 className="truncate text-sm font-bold text-white">SRM-GPT</h1>
          </div>
          <button
            onClick={() => { setSessionId(null); setMessages([]); }}
            className="focus-ring rounded-lg p-2 text-slate-300 hover:bg-slate-800"
            aria-label="New chat"
          >
            <Plus size={20} />
          </button>
